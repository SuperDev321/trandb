...
const headCells = [
  ...
  { id: 'from', numeric: false, disablePadding: false, label: 'Sender'},
...
export default function Logs() {
  ...
  const {room, type: roomType, from, to} = useContext(RoomContext);
  ...
  React.useEffect(() => {
    ...
    if(roomType === 'public' && room) {
        getPublicLogs()
    } else if(roomType === 'private' && from && to){
        console.log('private messages get', from ,to, roomType)
        getPrivateLogs()
    } else {
        setRows([]);
    }
    
  }, [room, roomType, from, to]);
  React.useEffect(() => {
    if(query !== '') {
